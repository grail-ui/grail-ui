...
function parseComment(comment: JSONOutput.Comment) {
  ...
  return {
    description: summary.length
      ? summary
          .map((summary) => (summary.kind === 'inline-tag' ? `{${summary.tag} ${summary.text}}` : summary.text))
          .join('')
      : undefined,
    blockTags: blockTags.map((tag: any) => ({
      name: tag.name ?? tag.tag.replace(/@/, ''),
      text: tag.content
        .map((content: any) => (content.kind === 'inline-tag' ? `{${content.tag} ${content.text}}` : content.text))
        .join(''),
    })),
    modifierTags,
  };
...
function renderMethod(node: JSONOutput.DeclarationReflection): string {
  ...
  return node.signatures.map((signature) => `${node.name}${renderSignature(signature, false)}`).join('\n');
}
