...
describe('Accordion', () => {
  const setup = (props: Partial<AccordionConfig> & { items?: AccordionItemParams[] } = defaultProps) => {
  ...
  describe('multiple', () => {
    ...
    describe('when clicking a trigger', () => {
      beforeEach(async () => {
        ({ container, root, triggers } = setup({ type: 'multiple', onValueChange: onValueChangeSpy }));
        await userEvent.click(triggers[0]);
      });
