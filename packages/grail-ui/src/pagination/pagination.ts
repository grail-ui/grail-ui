...
export const createPagination = ({
  ...
  const count$ = derived<[Writable<number>, Writable<number>], number>([total$, perPage$], ([$total, $perPage]) => {
  ...
  const pageAttrs = derived(page$, () => {
    return function (item: PaginationItem) {
      return {
        ...(item.page && !item.disabled ? { 'aria-label': getPageAriaLabel(item.page, item.selected) } : {}),
