...
export const createProgress = ({
  ...
  const clampedValue$ = derived([value$], ([$value]) => ($value === null ? null : clamp($value, minValue, maxValue)));
  ...
  const valueLabel = derived([clampedValue$, percentage], ([$value, $percentage]) => {
    ...
    return formatValueLabel({ value: $value, percentage: $percentage as number, minValue, maxValue });
  });
